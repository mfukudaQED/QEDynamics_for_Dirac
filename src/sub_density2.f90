...
!  !psi_S \sigma^l D_k \psi_L
!  if(l==1) then
!           tau_pq_kl = conjg(psi_p(4))*dpsi_q(k,1) +conjg(psi_p(3))*dpsi_q(k,2)
!  else if(l==2) then
!           tau_pq_kl = IU*conjg(psi_p(4))*dpsi_q(k,1) -IU*conjg(psi_p(3))*dpsi_q(k,2)
!  else if(l==3) then
!           tau_pq_kl = conjg(psi_p(3))*dpsi_q(k,1) -conjg(psi_p(4))*dpsi_q(k,2)
!  end if

!!psi_L \sigma^l D_k \psi_S
!  if(l==1) then
!           tau_pq_kl = conjg(psi_p(2))*dpsi_q(k,3) +conjg(psi_p(1))*dpsi_q(k,4)
!  else if(l==2) then
!           tau_pq_kl = IU*conjg(psi_p(2))*dpsi_q(k,3) -IU*conjg(psi_p(1))*dpsi_q(k,4)
!  else if(l==3) then
!           tau_pq_kl = conjg(psi_p(1))*dpsi_q(k,3) -conjg(psi_p(2))*dpsi_q(k,4)
!  end if
