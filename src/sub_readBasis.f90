 ...
 do inat=1,NAT
  ...
  if(Atom.ne.'Atom') then
   ...
   read(11,'(a4,6x,i3,x,3f20.10)') Atom, atomicnum, tmpx, tmpy, tmpz
    ...
    zc(inat)=tmpz
    cn(inat)=atomicnum
   write(*,'(a,i3,a,i6,3es23.15)') 'cn & xyzc of atom(',inat,')',cn(inat),xc(inat),yc(inat),zc(inat)
...
!----------count the number of large basis-----------------------------------
   ...
   do
      ...
      do ! loopA
        read(11,'(a5)') MOJI ! set MOJI = orbital angular momentum term symbol here
        if(MOJI.eq.'Atom ') then
          ...
          do i=1,5
             if(MOJI(i:i)>='A' .and. MOJI(i:i)<='Z') then ! MOJI(i:i) -> orbital angular momentum
          ...
          countNBS_L = countNBS_L + numShell ! repeat over the number of contractions
        end if
...
!----------count the number of small basis-----------------------------------
   ...
   NBS_SS = NBS_S**2
   NBS_LS = NBS_L*NBS_S
   NBS_LT = NBS_L*(NBS_L+1)/2
   NBS_ST = NBS_S*(NBS_S+1)/2
   NBS_00 = NBS0**2
   ! ADDED END
...
!=============================================================================
  ...
  character(LEN=5) pg_orbital
   ...
   if (pg_orbital.eq.'    S') then
      ...
      num = 6
   else if (pg_orbital.eq.'    K') then
      num = 7
   else
      write(*,*)'error pg_orbital in subroutine ShellNumber'
   end if

   numShell = (num+1)*(num+2)/2      ! 1 3 6 10 15 21 28 36
                                     ! S P D  F  G  H  I  K
   sumShell = num*(num+1)*(num+2)/6  ! 0 1 4 10 20 35 56 84
                                     ! S P D  F  G  H  I  K
   numNs = sumShell
   numN2 = num
