...
!   setQmat_TM(TM_Qmat)
!   setQmat_VT(VT_Qmat) [131123]
!   setQmat_M(M_Qmat) [131123]
!   setQmat_h(h_Qmat)
!====================================================================
...
!======================================================
  ...
  do j=1,Nph
     !$omp parallel do default(none), shared(calFj0_Qmat,j,NBS),private(nn,mm,n,m,a,b)
     do nn=1,4*NBS
        do mm=1,4*NBS
...
!======================================================
  ...
  complex(kind=dp),intent(out) :: VT_Qmat(4*NBS,4*NBS)

  complex(kind=dp) :: intVT_mat
  integer :: nn,mm  ! index for 1~4*NBS
  integer :: n,m ! index for 1~2*NBS (+ or - specified)
  character(LEN=1) :: a,b
  ...
  do nn=1,4*NBS
     do mm=1,4*NBS
        ...
        VT_Qmat(nn,mm) = intVT_mat(n,a,m,b)
        
     end do
  end do

end subroutine setQmat_VT

!======================================================
subroutine setQmat_M(M_Qmat)
! 1 ~ 2*NBS : "+"
! 2*NBS +1 ~ 2*NBS : "-"
! in "+" and "-", 1,1bar,2,2bar, ... respectively
!======================================================
  ...
  complex(kind=dp),intent(out) :: M_Qmat(4*NBS,4*NBS)
  ...
  do nn=1,4*NBS
     do mm=1,4*NBS
        ...
        M_Qmat(nn,mm) = intM_mat(n,a,m,b)
        
     end do
  end do

end subroutine setQmat_M
...
!======================================================
  ...
  !$omp parallel do default(none), shared(h_Qmat,NBS),private(nn,mm,n,m,a,b)
  do nn=1,4*NBS
     do mm=1,4*NBS
